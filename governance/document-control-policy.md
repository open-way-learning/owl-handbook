...
* **GitBook (backed by GitHub)** holds OWL’s canonical, version-controlled documentation <kbd>–</kbd> our core policies, procedures, templates, and frameworks that describe how we operate.
...
{% hint style="warning" %}
**Note:** GitBook is paired with GitHub behind the scenes so that changes to official documents are fully version-controlled and easy to track over time. See [Why GitHub/GitBook?](document-control-policy.md#why-github-gitbook) for more background.
{% endhint %}
...
* Proposed changes are surfaced through the open-source mechanisms described in this policy so that Stewards (see below) and relevant teammates can adjust the “source of truth” for everyone.

In other words: _we either follow the current rule or work together to change the rule – no exceptions._ Quietly bypassing our agreed-upon structures is _not_ acceptable and undermines our ability to act as a coherent, trustworthy organization.
...
> _GitBook is the "operating system." Google Drive is the "workbench."_
...
In short, Google Drive is where someone goes to answer the question “What are we working on right now and with whom?”
...
See [Appendix 1: Folder Map & Stewardship Overview](document-control-policy.md#appendix-1-folder-map-and-stewardship-overview) for a detailed map of how this plays out across OWL's Google Drive and GitBook structures.
...
* Workflow 1: Create a _new official_ OWL document (goes to GitBook)
* Workflow 2: Suggest changes to an _existing_ GitBook document
* Workflow 3: Create or use _working documents_ that stay in Google Drive
...
2. **Make your suggested changes to the document in question.** At minimum, be sure to include what exactly you recommend changing, why it matters, and the proposed wording and format. You will do this by using one of the following three methods:
   ...
   * **For larger edits or rewrites:** Use GitBook's [change request process](https://gitbook.com/docs/collaboration/change-requests) to initiate a change. Create the change you wish to see, then add the appropriate section Steward as a reviewer on your change request.
3. **Steward review & collaboration.** The Steward reviews the suggestion and may invite others into the conversation, especially for significant changes (For bigger decisions, the Steward follows OWL's [Collective Leadership and Decision Making RACI procedure](../Strategy-Innovation/collective-leadership-and-decision-making.md#owls-raci-s-table) so that input and accountability are clear). At the end of the appropriate review process, the Steward and proposer will agree on a reasonable timeline for finalizing the update.
...
*   **01\_Internal\_Business-Ops: How OWL runs internally – strategy, governance, finance, HR, operations, outreach, development, staff learning, and data.**
    ...
    * **03\_Outreach & Programming:** How OWL designs, communicates, and coordinates offerings with the outside world. Planning and messaging, not data storage. Note: _Actual impact data and evaluation artifacts do not live here; they belong in_ 06\_Data, Learning & Impact.
      * Steward: Director of Program Impact & Visibility (DPIV)
      * Typical contents:
...
**Rule of Thumb:** When naming or saving a file, ask: _“If someone else needed to find this six months from now, what keywords would help them locate it quickly?”_
...
In short, this combination of GitHub and GitBook helps OWL accomplish an important goal: maintain transparent, auditable, collaborative documentation of its key processes, policies, and procedures—and do so in a public-by-default manner aligned with OWL's open ethos.

#### Do we record SOPs in the handbook?

Yes. Storing standard operating procedures are the GitBook handbook's _raison d'être_. These are precisely the kinds of canonical, organizationally applicable documents that the system is designed to house and assist in modifying.
...
#### How do we safely revert a mistake? <a href="#docs-internal-guid-a73473a8-7fff-da4e-0c6d-3342d0b58f1d" id="docs-internal-guid-a73473a8-7fff-da4e-0c6d-3342d0b58f1d"></a>
...
{% hint style="warning" %}
In Git-based workflows, time only moves forward. So while you are "rolling back" a change, in the project history that rollback is actually logged as a separate change _after_ the one you revert. That means it is actually possible to rollback the rollback!
{% endhint %}

#### How do we know we're using the handbook well?

We'll know OWL is making best use of our handbook when we observe [bias for action](https://handbook.gitlab.com/teamops/decision-velocity/#bias-for-action) amongst team members. That means they are:
...
* Always responding with a link—that is, when answering someone's question that sounds like "What do we do about…," their responses sound something like "Our process is this, and it's documented here: \[link to handbook section and sub-section]."
...
{% hint style="success" %}
This section is optional for GitHub-savvy contributors and Tech Leads.
{% endhint %}
